...
class FeatureSelector(BaseEstimator):
    ...
    def shap_importance_analysis_round2(
        ...
        if cache_explainer:
            try:
                ...
                # For interventional mode, we need background data
                if self.shap_feature_perturbation == 'interventional':
                    rng_base = np.random.default_rng(self.random_state)
                    background_size = min(100, len(X))
                    background_idx = rng_base.choice(len(X), size=background_size, replace=False)
                    background_data = X.iloc[background_idx]
        ...
        def _single_shap_run(run_idx: int):
            ...
            if base_explainer is None:
                ...
                # For interventional mode, we need background data
                if self.shap_feature_perturbation == 'interventional':
                    # Use a subsample as background data for interventional SHAP
                    background_size = min(100, len(X_sample))
                    background_idx = rng_run.choice(len(X_sample), size=background_size, replace=False)
                    background_data = X_sample.iloc[background_idx]
                    explainer = shap.TreeExplainer(
                        model,
                        data=background_data,
                        feature_perturbation=self.shap_feature_perturbation,
    ...
    def null_importance_ultimate(
        ...
        """
        Null Importance with Block Bootstrap for time series (NO TEMPORAL LEAKAGE).

        Compares actual feature importance against null distribution using
        circular shift (Block Bootstrap) instead of random shuffle to preserve
        temporal dependencies.

        SAFETY: Uses np.roll for permutation - preserves temporal autocorrelation.

        Args:
            X: Training features
            y: Training target
            n_actual: Number of actual runs (default 15)
            n_null: Number of null runs (default 75)

        Returns:
            Dict with importance scores and statistical significance
    ...
    def adversarial_validation(
        ...
        """
        Adversarial Validation for temporal drift detection (NO DATA LEAKAGE).

        Splits training data into early/late periods and trains a classifier
        to distinguish them. High AUC indicates distribution shift over time.

        SAFETY: This method NEVER uses actual test data to prevent future leakage.

        Args:
            X_train: Training features only

        Returns:
            Dict with 'adv_importance' and 'high_shift' features

        Reference:
            - Fastai Adversarial Validation
        ...
        # Safety assertion: ensure we're not accidentally passed test data
        assert isinstance(X_train, pd.DataFrame), "X_train must be DataFrame"
        assert len(X_train) > 100, "X_train too small for adversarial validation"
    ...
    def process_batch(
        ...
        # ═══════════════════════════════════════════════════════════════════
        # CRITICAL SAFETY CHECK: Temporal Split BEFORE Any Preprocessing
        # ═══════════════════════════════════════════════════════════════════
        ...
        # Safety Assertion: Ensure temporal ordering is preserved
        ...
        # Safety Assertion: Test data comes AFTER train data (temporal ordering)
        # This prevents future leakage by ensuring chronological order
        ...
        # Safety Assertion: Test uses same features as train (no new features)
...
def main():
    ...
    if 'target' not in df.columns:
        # Try to load target from F2.parquet (full dataset with 3886 features)
        logging.warning('No target column in F.parquet, trying to load from F2.parquet...')
        try:
            target_df = pd.read_parquet('F2.parquet', columns=['target'])
            if len(target_df) == len(df):
                df['target'] = target_df['target'].values
                logging.info(f'Target loaded from F2.parquet successfully!')
            else:
