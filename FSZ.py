...
class FeatureSelector(BaseEstimator):
    ...
    def train_and_save_final_model(self, X: pd.DataFrame, y: pd.Series, output_dir: str, batch_id: int = 0):
        # Disabled: No model output needed
    ...
    def _calculate_optimal_ts_cv_params(self, n_samples: int) -> Dict:
        ...
        # Log only once to avoid repetition
    ...
    def null_importance_ultimate(
        ...
        for run in range(n_null):
            ...
            block_size = max(1, int(np.sqrt(len(y_train))))
    ...
    def adversarial_validation(
        ...
        y_pred_proba = model.predict(X_combined.iloc[train_size:])
        auc_score = roc_auc_score(y_combined[train_size:], y_pred_proba)
        logging.info(f'\n{"-"*70}')
    ...
    def quick_prefilter(self, X: pd.DataFrame, y: pd.Series, n_estimators: int = 100) -> tuple:
        ...
        logging.info(f'[OK] Quick pre-filter: {X.shape[1]} -> {X_filtered.shape[1]} features (-{len(dropped_features)})')
    ...
    def process_batch(
        ...
        adversarial = self.adversarial_validation(X_train)
        ...
        # Filter out dropped features (score=0) and show only meaningful features
        top_10 = df_ranking[df_ranking['category'] != 'dropped'].nlargest(10, 'score')[['feature', 'score', 'category']]
        for idx, row in top_10.iterrows():
            logging.info(f'   {row["category"][:6]:6s} | {row["score"]:7.4f} | {row["feature"][:50]}')
        logging.info(f'{"="*70}')
        ...
        # Check 1: Null Importance Quality
        if null_importance:
    ...
    def _save(self, df_ranking: pd.DataFrame, categorized: Dict, cv_metrics: Dict, batch_id: int, output_dir: str, execution_time: float = 0, memory_used: float = 0, memory_start: Optional[float] = None, memory_end: Optional[float] = None, process_pid: Optional[int] = None):
        ...
        # Save only 3 parquet files: FSZ_strong, FSZ_medium, FSZ_weak
